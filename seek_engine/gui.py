...
class SEMHybridEngineApp:
    ...
    def _on_mapping_complete(self, targets):
        self._clear_editor()
        self.target_text.insert(tk.END, f"# Extracted {len(targets)} targets for {self.target_ip_var.get()}\n")
    ...
    def _start_batch(self):
        ...
        if not targets: return self.log("[Error] Target Editor is empty.")
        
        self.btn_run.config(state=tk.DISABLED); self.btn_disc.config(state=tk.DISABLED)
        self.btn_abort.config(state=tk.NORMAL)
        self.progress['maximum'] = len(targets)
        self.progress['value'] = 0
        
        self.engine.start_batch(targets, self.delay_var.get(), self.send_o_var.get())
    ...
    def _start_discovery(self):
        ...
        self.btn_run.config(state=tk.DISABLED); self.btn_disc.config(state=tk.DISABLED)
        self.btn_save.config(state=tk.NORMAL); self.btn_skip.config(state=tk.NORMAL)
        
        self.engine.start_discovery(targets, self.delay_var.get())
    ...
    def _disc_save(self): self.engine.save_and_next()
    def _disc_skip(self): self.engine.skip_and_next()
    def _abort_campaign(self): self.engine.abort()